...
Repository로 부터 최신 Chart를 다운로드 하기 위해서는 애플리케이션 배포 이전에 repository를 업데이트해주는 것이 좋다.
```
$ helm repo update
...
Helm Chart를 찾아 배포하는 여러 가지 방법이 있지만, 시작하기 가장 좋은 방법은 위에서 구성한 official stable chart 중 하나를 사용하여 배포하는 것이다.

여기서는 `mysql`을 예시로 배포해본다.
...
### Helm Chart를 이용하여 애플리케이션 배포

이제 helm chart를 이용하여 MySQL 애플리케이션을 배포해보자.
```
...
위의 예에서 `stable/mysql` chart가 배포되었고, 이름은 `mysql-1604311201`인 것을 확인할 수 있다. 

Helm chart로 배포한 애플리케이션을 조회해보자.
```
$ helm ls
...
애플리케이션 배포를 위해 필요한 모든 kubernetes 리소스가 함께 배포된 것을 볼 수 있다.

이렇게 Helm chart를 사용하여 배포를 하게 되면 `새로운 releade`가 된것이다. 즉 하나의 Helm chart로 여러번의 배포를 할 수 있는데, 이때 각각의 배포는 덮어씌어지는 것이 아니라 독립적으로 관리되고 업그레이드된다.

그렇다면 실제로 그렇게 동작하는지 테스트해보자. 위와 동일한 방식으로 mysql을 하나 더 배포한다.
```
$ helm install stable/mysql --generate-name
...
`kubectl get all`로 조회해도 마찬가지로 독립적인 애플리케이션이 배포되었음을 알 수 있다.
```
$ kubectl get all
NAME                                    READY   STATUS    RESTARTS   AGE
...
### Helm Chart를 이용하여 배포한 애플리케이션 제거
Helm chart를 이용하여 배포한 애플리케이션을 제거하는 방법은 아주 간단하다.
```
